...
const Toggler = ({ options, activeIndex, onToggle }) => {
  ...
  return (
    <FontSizeContext.Consumer>
      {({ fontSize }) => (
        <Box borderRadius="1.5em" bg="rgba(255,255,255,0.2)" py="0.75em" px="0.5em">
          <Box position="relative">
            <Box
              position="absolute"
              left={poses[activeIndex][0].left - poses[0][0].left - fontSize * 0.5}
              top="50%"
              bottom="0"
              width={poses[activeIndex][0].width + fontSize}
            ...
            <Box position="relative">
              {options.map((label, i) => (
                <Measure bounds onResize={({ bounds }) => poses[i][1](bounds)}>
                  {({ measureRef }) => (
                    <Box.inline key={i} ref={measureRef} px="1.25em" color="white" onClick={() => onToggle(i)}>
                      {label}
                    </Box.inline>
                  )}
                </Measure>
